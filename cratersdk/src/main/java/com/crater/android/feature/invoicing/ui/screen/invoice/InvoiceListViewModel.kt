...
class InvoiceListViewModel @Inject constructor(
    ...
    fun fetchInvoiceDetails() {
        viewModelScope.launch(exceptionHandler) {
            ...
            val invoiceTrackerDeferred = async { repository.getInvoiceTracker() }
            val recentInvoicesDeferred = async { repository.getRecentInvoices() }
            _uiState.update {
                it.copy(
                    isLoading = false,
                    invoiceTracker = invoiceTrackerDeferred.await(),
                    recentInvoiceList = recentInvoicesDeferred.await()
                )
            }
        }
    }
